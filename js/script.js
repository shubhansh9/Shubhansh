/* =============================================
   CURSOR DOT — hide on touch devices
============================================= */
...
(function buildLoader() {
  ...
  const slotHeight = window.innerWidth < 480 ? 35 : window.innerWidth < 768 ? 40 : 48;
  ...
  slots.forEach(({ inner, targetIdx }, i) => {
    ...
    const delay = i * 60;        // ms stagger between letters (was 120)
    const stepDuration = 18;      // ms per alphabet step — very fast scroll (was 60)
    const totalSteps = targetIdx;
  ...
  const hideDelay = (NAME.length - 1) * 60 + 26 * 18 + 500;
  setTimeout(() => {
